    ...
    <>
      <div className='flex justify-center items-center h-screen'>
        <div className='w-2/4 h-2/4 bg-gray-200 rounded'>
          <h1 className='text-center font-semibold my-2'>Inserisci il titolo</h1>
          <form className='flex justify-around w-2/4 mx-auto' onSubmit={handleSubmit}>
            <input className='w-3/4 border-solid border-2 border-black rounded' type="text" name="title" placeholder='inserisci il titolo' value={formData.title} onChange={(e) => updateFormData(e.target.value, 'title')} />
          ...
          <div className='w-full mt-10'>
            <ul className='w-full'>
              {titleList.map((elem, i) => {
                return (
                  <li key={i} className='flex justify-between border-l-4 border-green-600 w-2/4 mx-auto my-3 pl-2'>{elem.title}
